...
public class NotLikePredicate<T> extends PredictResolver {
    public NotLikePredicate(Class<T> clazz, SearchCriteria criteria) {
    ...
    public BooleanExpression getPredicate() throws Exception {
        ...
        for (String value : values) {
            ...
            if (ex == null) {
                ex = entityPath.getString(targetProp).notLike(tempValue);
            } else {
                ex = ex.or(entityPath.getString(targetProp).notLike(tempValue));
            }
        }
        return ex;
    }
